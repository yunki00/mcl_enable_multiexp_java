...
class Fr {
	...
	friend void mulVec(G1& z, const std::vector<G1>& x, const std::vector<Fr>& y);
	friend void mulVec(G2& z, const std::vector<G2>& x, const std::vector<Fr>& y);
	friend void powVec(GT& z, const std::vector<GT>& x, const std::vector<Fr>& y);
public:
	Fr() {}
...
void mulVec(G1& z, const std::vector<G1>& x, const std::vector<Fr>& y)
{
	if (x.size() != y.size()) {
		throw std::runtime_error("mulVec: input vector sizes do not match");
	}
	if (x.empty()) {
		z.clear();
		return;
	...
	std::vector<mcl::bn::G1> xVec(x.size());
	std::vector<mcl::bn::Fr> yVec(y.size());
	for (size_t i = 0; i < x.size(); i++) {
		xVec[i] = x[i].self_;
		yVec[i] = y[i].self_;
	}

	mcl::bn::G1::mulVec(z.self_, xVec.data(), yVec.data(), x.size());
}
/*
	#G2 = r
*/
...
void mulVec(G2& z, const std::vector<G2>& x, const std::vector<Fr>& y)
{
	if (x.size() != y.size()) {
		throw std::runtime_error("mulVec: input vector sizes do not match");
	}
	...
	std::vector<mcl::bn::G2> xVec(x.size());
	std::vector<mcl::bn::Fr> yVec(y.size());
	for (size_t i = 0; i < x.size(); i++) {
		xVec[i] = x[i].self_;
		yVec[i] = y[i].self_;
	...
	mcl::bn::G2::mulVec(z.self_, xVec.data(), yVec.data(), x.size());
}
...
void powVec(GT& z, const std::vector<GT>& x, const std::vector<Fr>& y)
{
	if (x.size() != y.size()) {
		throw std::runtime_error("powVec: input vector sizes do not match");
	}
	if (x.empty()) {
		z.clear();
		return;
	...
	std::vector<mcl::bn::GT> xVec(x.size());
	std::vector<mcl::bn::Fr> yVec(y.size());
	for (size_t i = 0; i < x.size(); i++) {
		xVec[i] = x[i].self_;
		yVec[i] = y[i].self_;
	...
	mcl::bn::GT::powVec(z.self_, xVec.data(), yVec.data(), x.size());
}
